 ...
 return <main>
  ...
  <div className="page">
   ...
   <div className="columns">
    <section className="archive" id="timeline"><div className="heading"><h2>エイプリルフール企画年表</h2><small>{list.length}件を表示中</small></div><div className="timeline">{[...new Set(list.map(x=>x.year))].map(year=><section className="yearGroup" id={`year-${year}`} key={year}><h3 className="yearTitle">{year}年</h3>{list.filter(x=>x.year===year).map(x=><article className="entry" key={x.year+x.title}><div className="card"><div className="body"><div className="badges"><span>{x.kind}</span>{x.after&&<span className="gold">嘘から現実へ</span>}</div><div className="titleBlock"><span>企画名</span><h3>{x.title}</h3></div><p>{x.text}</p>{x.note&&<p className="note">※ {x.note}</p>}{x.video&&<a className="thumb" href={x.url} target="_blank"><img src={`https://i.ytimg.com/vi/${x.video}/hqdefault.jpg`} alt={x.title}/></a>}{x.after&&<div className="after"><b>その後</b><p>{x.after}</p></div>}<dl className="sourceDetails"><div><dt>場所</dt><dd>{x.region}</dd></div><div><dt>URL</dt><dd><a href={x.url} target="_blank">{x.url}</a></dd></div></dl></div></div></article>)}</section>)}</div>
