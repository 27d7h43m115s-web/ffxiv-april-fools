 ...
 return <main>
  ...
  <div className="page">
   ...
   <div className="columns">
    <section className="archive" id="timeline"><div className="heading"><h2>エイプリルフール企画年表</h2><small>{list.length}件を表示中</small></div><div className="timeline">{list.map(x=><article className="entry" id={`year-${x.year}`} key={x.year+x.title}><div className="entryHead"><time>{x.year}</time><div className="badges"><span>{x.region}</span><span>{x.kind}</span>{x.after&&<span className="gold">嘘から現実へ</span>}</div></div><div className="card">{x.video&&<a className="thumb" href={x.url} target="_blank"><img src={`https://i.ytimg.com/vi/${x.video}/hqdefault.jpg`} alt={x.title}/></a>}<div className="body"><h3>{x.title}</h3><p>{x.text}</p>{x.after&&<p className="after"><b>その後：</b>{x.after}</p>}{x.note&&<p className="note">※ {x.note}</p>}<a className="official" href={x.url} target="_blank">公式資料を見る&nbsp; ↗</a></div></div></article>)}</div>
